...
rownames(expressionMatrix) < rownames(counts_data)
...
output_hits <- merge(counts_data$Gene.Name, topfit, by.y=0, by.x=1, all.y=TRUE)
output_hits <- output_hits[order(output_hits$P.Value),]

kable(output_hits,type="html",row.names = FALSE)
```
```{r p_value_pass}
