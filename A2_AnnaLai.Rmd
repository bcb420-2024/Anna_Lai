...
packages_to_install <- c("BiocManager", "RSQLite", "readr", "gplots", "reshape2", "RColorBrewer", "grid", "BiocManager", "knitr", "kableExtra", "devtools", "colorRamp2")
...
kable(head(qlf_output_hits), caption="Quasi Liklihood method results group by condition type") %>% kableExtra::kable_styling()
```
...
kable(head(qlf_output_hits), caption="Quasi Liklihood method results group by cell line") %>% kableExtra::kable_styling()
```
...
```{r}
condition_type_grouped <- factor(exp_groups$condition_type, levels = unique(exp_groups$condition_type))

condition_type_grouped
```


```{r}
# Reorganize heatmap_matrix_tophits based on exp_groups$cell_line


# Create annotations
top_annotation <- HeatmapAnnotation(foo = anno_block(gp = gpar(fill = 2:length(unique(exp_groups$cell_line)))) )
```
...
name = "exression level",                         cluster_rows = TRUE,
...
top_annotation = HeatmapAnnotation(foo = anno_block(gp = gpar(fill = color_vector, col = NA)))
)# Heatmap
